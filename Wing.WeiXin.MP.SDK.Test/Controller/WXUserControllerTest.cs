...
//            Assert.AreEqual(GetWXGroupByWXUserTest(newUser).group.id, groupNew.group.id);
        ...
        [TestMethod]
        public void GetWXUserListTest()
        {
            WXUserList l = GlobalManager.FunctionManager.WXUser.GetWXUserList(account);
        ...
        {
            Assert.AreEqual(GlobalManager.FunctionManager.WXUser.GetWXUser(account, openID).openid, openID);
        }

        public void AddWXGroupTest(WXUserGroup group)
        {
            Assert.AreEqual(GlobalManager.FunctionManager.WXUser.AddWXGroup(account, group).group.name, group.group.name);
        }

        [TestMethod]
        public WXUserGroupList GetWXUserGroupListTest()
        {
            return GlobalManager.FunctionManager.WXUser.GetWXUserGroupList(account);
        } 

        public void ModityGroupNameTest(WXUserGroup group)
        {
            Assert.AreEqual(GlobalManager.FunctionManager.WXUser.ModityGroupName(account, group).errcode, "0");
        } 

        public void MoveGroupTest(WXUser user, WXUserGroup group)
        {
            Assert.AreEqual(GlobalManager.FunctionManager.WXUser.MoveGroup(account, user.openid, group.group.id).errcode, "0");
        } 

        public WXUserGroup GetWXGroupByWXUserTest(WXUser user)
        {
            return GlobalManager.FunctionManager.WXUser.GetWXGroupByWXUser(account, user);
        } 

        public WXUser GetWXUserListFromListTest(WXUserList userList)
        {
            return GlobalManager.FunctionManager.WXUser.GetWXUserListFromList(account, userList, 1)[0];
        } 

        [TestMethod]
        public void GetWXGroupByWXUserTest()
        {
            WXUserGroup g = GlobalManager.FunctionManager.WXUser.GetWXGroupByWXUser(account, new WXUser
            {
        ...
        [TestMethod]
        public void BatchGetWXUserTest()
        {
            BatchWXUserList a = GlobalManager.FunctionManager.WXUser.BatchGetWXUser(account, new BatchWXUser
            {
                user_list = new List<BatchWXUser.BatchWXUserItem>
                {
                    new BatchWXUser.BatchWXUserItem{ openid = "orImOuC33jQiJFrVelQGGTmwPSFE"}
                }
            });
